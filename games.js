...
const SUPPORTED_FORMATS = ['windows', 'mac', 'linux']
...
commander
  ...
  .option('-f, --format <format>', util.format('What format to download the ebook in (%s)', ALLOWED_FORMATS.join(', ')), 'windows')
    ...
    request.get({
      ...
      async.concat(response.body, (item, next) => {
        ...
        const filteredOrders = orders.filter((order) => {
          ...
          if (flatten(keypath.get(order, 'subproducts.[].downloads.[].name')).indexOf('download') !== -1) {
            fitler = false;
          } else {

            for (format of SUPPORTED_FORMATS) {
              if (flatten(keypath.get(order, 'subproducts.[].downloads.[].platform')).indexOf(format) !== -1) {
                filter = true;
              }
            }
            if (commander.format !== 'all' && flatten(keypath.get(order, 'subproducts.[].downloads.[].platform')).indexOf(commander.format)) {
              fitler = true;
            }
          }
          return filter;
...
function downloadGame (bundle, name, download, callback) {
...
function downloadBundles (next, bundles) {
  ...
  for (var bundle of bundles) {
    ...
    for (var subproduct of bundle.subproducts) {
      var filteredDownloads = subproduct.downloads.filter((download) => {
        return (download.platform === 'windows' || download.platform === 'mac' || download.platform === 'linux') &&
          (commander.format !== 'all' && download.platform === commander.format)
      })

      var downloadStructs = flatten(keypath.get(filteredDownloads, '[].download_struct'))

      for (var filteredDownload of downloadStructs) {
  ...
  async.each(downloads, (download, next) => {
    limiter.submit((next) => {
      ...
      downloadGame(download.bundle, download.name, download.download, (error, skipped) => {
        if (error) {
          return next(error)
        }
